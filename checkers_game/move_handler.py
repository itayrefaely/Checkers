...
class MoveHandler:
    ...
    def initialize_new_move(self, board, ui):
        ...
        This method checks if any black pawns have capturing moves. If so, it highlights them.
        If there are no capturing moves, it highlights black pawns with possible moves.
        """
        red_pawns_with_capture = self.find_pawns_with_capture_moves(board, board.black_team)

        if red_pawns_with_capture:
            for pawn in red_pawns_with_capture.keys():
                pawn.highlight(ui)
        else:
            self.highlight_pawns_with_possible_moves(board, board.black_team, ui)
    ...
    @staticmethod
    def highlight_pawns_with_possible_moves(board, pawn_team, ui):
        for pawn in pawn_team:
            if pawn.get_next_squares(board):
                pawn.highlight(ui)

    def is_legal_press(self, board, pos, is_first_press, ui):
        """
        ...
        if is_first_press:
            return self.handle_first_press(board, square_number, ui)
        else:
            return self.handle_second_press(board, square_number, ui)

    def handle_first_press(self, board, square_number, ui):
        """
        ...
        """
        chosen_pawn = self.find_chosen_pawn(board.black_team, square_number)
        ...
        self.unhighlight_all(board.black_team, ui)
        self.select(board, chosen_pawn, next_squares, ui)
        return True

    def handle_second_press(self, board, square_number, ui):
        """
        ...
        """
        chosen_pawn, prev_pawn = self.find_prev_and_cur_pawns(board.black_team, square_number)

        # Check if a black pawn was pressed again
        if chosen_pawn and prev_pawn:
            return self.handle_second_press_pawn(board, chosen_pawn, prev_pawn, ui)

        # If pressed a selected square
        elif board.squares[square_number].selected:
            return self.handle_second_press_square(board, prev_pawn, square_number, ui)

        return False

    def handle_second_press_pawn(self, board, chosen_pawn, prev_pawn, ui):
        """
        ...
        self.deselect(board, prev_pawn, prev_next_squares, ui)
    ...
    def handle_second_press_square(self, board, prev_pawn, square_number, ui):
        """
        ...
        if prev_pawn.can_eat:
            prev_next_squares = prev_pawn.get_next_squares(board)
            self.deselect(board, prev_pawn, prev_next_squares, ui)
            self.eat(board, prev_pawn, square_number, ui)
            ...
            if next_jumping_squares:
                ...
                self.select(board, prev_pawn, next_jumping_squares, ui)
                return False
        ...
        # Not an eating move
            ...
            prev_next_squares = prev_pawn.get_next_squares(board)
            self.deselect(board, prev_pawn, prev_next_squares, ui)
            self.move(board, prev_pawn, square_number, ui)
            self.is_player_turn = False
            return True
    ...
    @staticmethod
    def unhighlight_all(pawn_team, ui):
        """
    ...
    @staticmethod
    def select(board, pawn, next_squares, ui):
        # Select the pawn
        pawn.select(ui)
        ...
        for square_number in next_squares:
            ...
            square.select(pawn.radius, ui)

        pygame.display.flip()

    @staticmethod
    def deselect(board, pawn, next_squares, ui):
        # Deselect the pawn
        pawn.deselect()
    ...
    def move(self, board, pawn, next_square_number, ui):
        """
        ...
        self.update_move(board, pawn, next_square_number, ui)

    def eat(self, board, pawn, jump_square_number, ui):
        """
        ...
        self.update_move(board, pawn, jump_square_number, ui)

    @staticmethod
    def animate_movement(pawn, start_pos, end_pos, duration, start_time, clock, ui):
        """
    ...
    @staticmethod
    def delete_opponent_pawn(board, square_number, ui):
        """
        ...
        if opponent:
            board.delete_pawn(opponent, ui)

    @staticmethod
    def update_move(board, pawn, next_square_number, ui):
        # Update fields of pawn and it's new square
